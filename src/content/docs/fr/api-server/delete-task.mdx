...
<NotificationCard state="info" title="Note">
  `document.location.reload();` This is not best practice, but for simplicity we
  reload the page. In best practice, you would update the local state (signal or
  component data) by filtering out the deleted task from the current tasks
  array, avoiding the need to reload the entire page and providing a better user
  experience.
</NotificationCard>
